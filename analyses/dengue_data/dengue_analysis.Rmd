...
gene_scpca1 <- rotation_df %>%
  ...
  xtable(
    caption = "Genes with non-zero weights in first scPCA loadings vector.",
    digits = 4
  )
...
gene_scpca2 <- rotation_df %>%
  ...
  xtable(
    caption = "Genes with non-zero weights in second scPCA loadings vector.",
    digits = 4
  )
...
# see cluster_files folder
dengue_cpca_cv <- load(
  file = here("analyses/dengue_data/cluster_files/cpca_cv.Rdata")
)
...
```{r scPCA_cv}
# see cluster_files folder
dengue_scpca_cv <- load(
  file = here("analyses/dengue_data/cluster_files/scpca_cv.Rdata")
)
