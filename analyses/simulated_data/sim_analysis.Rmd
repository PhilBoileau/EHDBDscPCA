...
BiocParallel::register(SnowParam(workers = 3), default = TRUE)
sim_scpca_cv <- scPCA(tg, bg, center = TRUE, scale = TRUE, n_centers = 2,
                      parallel = TRUE, cv = 5)

# save the file for later
write_rds(sim_scpca_cv,
          path = here("analyses/simulations/scPCA_results/sim_scpca_cv.rds"))

# load the data
sim_scpca_cv <- read_rds(
  path = here("analyses/simulated_data/sim_scpca_cv.rds")
)
